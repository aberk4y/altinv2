...
const PortfolioPage = () => {
  ...
  const handleAddItem = async () => {
    if (newItem.name && newItem.quantity && newItem.buyPrice) {
      try {
        ...
        toast({ title: 'Basarili', description: 'Portfolye eklendi' });
      } catch (error) {
        console.error('Failed to add item:', error);
        toast({ title: 'Hata', description: 'Urun eklenemedi', variant: 'destructive' });
      }
    }
  };
  ...
  const handleDeleteItem = async (id) => {
    try {
      ...
      await fetchPortfolio();
      toast({ title: 'Basarili', description: 'Portfolyden silindi' });
    } catch (error) {
      console.error('Failed to delete item:', error);
      toast({ title: 'Hata', description: 'Urun silinemedi', variant: 'destructive' });
    }
  };
        ...
        {portfolio.length > 0 && (
          <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-2xl p-6 mb-6 text-white shadow-lg">
            ...
            <p className="text-3xl font-bold mb-3">
              {totalValue.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TL
            </p>
            <div className="flex items-center gap-2">
              {totalProfit >= 0 ? <TrendingUp size={18} /> : <TrendingDown size={18} />}
              <span className="font-semibold">
                {totalProfit >= 0 ? '+' : ''}{totalProfit.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TL
              </span>
                ...
                <div key={item.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-semibold">{item.buyPrice.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL</p>
                    </div>
                    <div>
                      <p className="text-gray-500">{t('currentPrice')}</p>
                      <p className="font-semibold">{currentPrice.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL</p>
                    </div>
                  </div>
                  <div className={`mt-3 pt-3 border-t flex items-center justify-between ${
                    ...
                    <div className="flex items-center gap-2 font-bold">
                      ...
                      <span>
                        {profit >= 0 ? '+' : ''}{profit.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL
                      </span>
